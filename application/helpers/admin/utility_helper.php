...
/*Function To Send An Email To Bulk
*/
function sendBulkMail($from = '',$to,$subject,$message)
{
	//if($from='')
	$from = 'no-reply@sebastian.com';
    include 'email_library.php'; // include the library file
...
/*
 * This helper works for multiple image upload
 * $PATH : Provide path where you want to upload images
  */
  function multiple_upload($FILE,$path,$num)
		{		
		for($i=0;$i<count($FILE['upload']['name']);$i++)
			{
			if(!move_uploaded_file($FILE['upload']['tmp_name'][$i],$path.$num))
				{
