...
class Ajax extends CI_Controller 
{
	...
	public function get_ajax_data()
	{
		...
		switch($flag)
		{
			...
			//This case is written to get Hotel Parent Services.
			case 19:{
				$output = $this->get_hotel_parent_services();
				echo json_encode($output);
				break;
			}
			//This case is written to get Latest 5 Service Requests from Hotel. 
			case 20:{
				$output = $this->get_guest_latest_requests();
				echo json_encode($output);
				break;
			}
			default:{
	...
	/*This method is used to get Hotel Latest 5 Service Requests From Hotel.
    * @param void
	* return array 
    */
	function get_guest_latest_requests(){
		$this->load->model('Services_model');
		$output = $this->Services_model->get_guest_latest_requests($this->input->post('service_id'));
	    return $output;
	}
	/*This method is used to get Hotel Parent Services
    * @param void
	* return array 
    */
	public function get_hotel_parent_services()
	{
		$this->load->model('Services_model');
		$output = $this->Services_model->get_hotel_unique_parent_services($this->session->userdata('logged_in_user')->sb_hotel_id);
		return $output;
	}	
